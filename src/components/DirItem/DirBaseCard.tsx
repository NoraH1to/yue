...
import { Box, Card, CardActionArea, CardActionAreaProps, Fade, Skeleton } from '@mui/material';
  ...
  <Box height={1}>
    <FixedRatio fixedWidth={4} fixedHeight={3}>
      <Card elevation={0} sx={{ height: 1, background: 'transparent' }}>
        <Skeleton variant="rounded" sx={{ width: 1, height: 1 }} animation="wave" />
      </Card>
    </FixedRatio>
...
export type DirItemDirBaseCardProps = { dirname: string } & Pick<CardActionAreaProps, 'onClick'>;

const DirItemDirBaseCard: FC<DirItemDirBaseCardProps> = ({ dirname, onClick }) => {
  return (
    <Fade in={true}>
